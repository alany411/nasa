...
export function ApodCard({ apod, loading = false, onOpen }: ApodCardProps) {
  if (loading || !apod) {
    return (
      <Card size="sm" className="rounded-sm ring-border">
  ...
  return (
    <button type="button" className="text-left" onClick={() => onOpen?.(apod.date)}>
      <Card size="sm" className="rounded-sm ring-border transition-colors hover:bg-accent/40">
        <div className="relative aspect-[3/2] overflow-hidden bg-muted">
          <img src={thumb} alt="" className="h-full w-full object-cover" />
          {apod.mediaType === 'video' ? (
            <span className="absolute top-2 left-2 rounded-sm bg-card px-1.5 py-0.5 text-[10px] font-semibold tracking-wide text-primary">
              VIDEO
            </span>
          ) : null}
        </div>
